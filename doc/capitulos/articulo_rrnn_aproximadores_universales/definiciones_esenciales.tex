...
\begin{teorema} [Teorema de independencia función de activación] 
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        $g = \sum_{j = 1} ^p  \beta_{gj} \cdot \prod_{k=1}^{l_{gj}}G(A_{gjk}(x))$ la suma de ambas seguirá la expresión
        de los elementos de $\pmcg$
...
Con esto lo que acabamos de probar que \textit{feedforward neural networks} $\sum \prod$ son capaces de aproximar cualquier 
función continua de variable real en un compacto.  


\subsection{Reflexión teorema de independencia función de activación}
Nota: En ningún momento dice que deba de ser una función de activación ¿Por qué pide entonces tales características en la definición de 
