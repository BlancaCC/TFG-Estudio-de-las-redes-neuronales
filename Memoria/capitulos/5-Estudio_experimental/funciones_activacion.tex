...
\begin{proof}
    ...
    dentro del otro necesariamente son iguales. 

    % Vamos a ver que tiene inversa la fi para poder hacerla isomorfa
    Por las hipótesis pedidas a $\phi$, ésta tiene inversa
    dada como 
    \begin{equation}
        \phi^{-1}((x,y)) =  
        \begin{bmatrix}
    ...
    Ante esto y repitiendo el razonamiento se tiene 
    la igualdad buscada: 
    \begin{equation*}
        \mathcal{H}^+_{\gamma, n}(\R^d, \R^s) 
        = 
...
\begin{corolario}[Mismo enunciado anterior pero con sigma y gamma afines]
    Hola yo soy un corolario. 
\end{corolario}
...
% Porqué se ha ampliado 
\marginpar{\maginLetterSize
    \iconoClave  \textcolor{darkRed}{    
        \textbf{
            Motivo de seleccionar estas agrupaciones
        }
    }
    
    Como ya comentamos en las conclusiones del 
    teorema \ref{teo:eficacia-funciones-activation}; 
    se puede aplicar en la práctica 
    como si dos funciones de activación con la
    misma forma (o muy parecidas) 
    producirán los mismos resultados (o muy parecidos). 
    De ahí que hayamos agrupado funciones 
