...
\end{teorema} 
  ...
  \marginpar{\maginLetterSize\raggedright
    \iconoAclaraciones \textcolor{dark_green}{ 
        \textbf{
            Idea intuitiva teorema
            \ref{teo:2_2_denso_función_continua}
        }
    }
  }
...
\marginpar{\maginLetterSize\raggedright
    \iconoAclaraciones \textcolor{dark_green}{ 
        \textbf{
            Idea intuitiva lema 
            \ref{lema:a_2_paso_previo_denso}   
        }
    }
}
\marginpar{\maginLetterSize
 Hasta ahora hemos probado que $\pmcg$ es capaz de aproximar funciones medibles pero siendo $G$ una función de activación continua, vamos a relajar tal hipótesis a que $G$ sea cualquier función de activación, sin exigirle continuidad.
  }
\normalmarginpar
\setlength{\marginparwidth}{\bigMarginSize}
% Lema A.2 
\begin{lema}\label{lema:a_2_paso_previo_denso}
    ...
    Para cualquier $\varepsilon > 0$ existe un elemento $H_{\varepsilon}$ de $\mathcal{H}_\psi{(\R, \R)}$ cumpliendo que
    \begin{equation}
        \sup_{\lambda \in \R} | F(\lambda) - H_{\varepsilon}(\lambda) | < \varepsilon.
    \end{equation}
\end{lema} 
\begin{proof}
    ...
    % Nota idea de demostración intuitiva  lema lema:a_2_paso_previo_denso
    \marginpar{\maginLetterSize\raggedright
        \iconoAclaraciones \textcolor{dark_green}{ 
            \textbf{
                Idea intuitiva demostración del lema 
                \ref{lema:a_2_paso_previo_denso}   
            }
        }
    }
    \marginpar{\maginLetterSize
    Aprovechamos que sabemos que la función de activación continua $F$ es no decreciente así que podemos dividir su imagen en trozos iguales y aproximar la imagen
    con una función en escalera que valga en cada tramo el valor mínimo.
    A su vez, una función en escalera se puede construir con una red neuronal de una capa oculta que tenga como función de 
    activación arbitraria $\psi$, \textit{cada vez que se activa un nodo más se sube un escalón más de de la escalera}.
    }
    \begin{itemize}
...
\end{teorema}
   ...
   % Nota idea  intuitiva  teorema teorema:2_3_uniformemente_denso_compactos
   \marginpar{\maginLetterSize\raggedright
   \iconoAclaraciones \textcolor{dark_green}{ 
       \textbf{
           Idea intuitiva teorema
...
\marginpar{\maginLetterSize
Con una versión generalizada de una red neuronal que utilice cualquier función de activación (es decir que pertenezca a $\rrnng$)
\textbf{ podemos aproximar funciones continuas y funciones medibles}.

Esto ha supuesto relajar la hipótesis de $G$ con respecto al último teorema \ref{teo:2_2_denso_función_continua} 
de convergencia a funciones.
}
\begin{proof}
...
\begin{proof}
    ...
    $\cos_{M, \frac{\varepsilon}{q B}} \in \mathcal{H}_\psi{(\R, \R)}$, 
    ...
    \begin{equation}
        ...
        &\leq
        \sup_{\lambda \in [-M, M]} 
        | \sum_{j=1}^q \beta_j \cos_{M, \frac{\varepsilon}{q B}}(\lambda )
         - 
...
\end{proof}
  ...
  \marginpar{\maginLetterSize
  \iconoAclaraciones \textcolor{dark_green}{ \textbf{Idea intuitiva lema \ref{lema:A_5_uniformemente_denso_compactos}:}}
  }
  \marginpar{\maginLetterSize
    Afirma que \textbf{podemos aproximar cualquier función continua definida 
    con una red neuronal independientemente de la 
    función de activación que use}. 
    Además, si los valores de entrada está 
    dentro de un rango acotado tendremos una cota de error única para todo el dominio.

    Podría pensarse que la capacidad de aproximación de una red neuronal depende de su función de activación, este resultado con matices lo desmiente, \textbf{eso sí, siempre que se tenga el número suficiente de neuronas}. 
  }
...
\end{teorema}
 ...
 \marginpar{\maginLetterSize
    \iconoAclaraciones \textcolor{dark_green}{ 
        \textbf{Idea intuitiva teorema \ref{teo:2_4_rrnn_densas_M}:}}
    }
    \marginpar{\maginLetterSize
    Afirma que las \textbf{redes neuronales son capaces de aproximar funciones medibles},
    es decir funciones muy generales con formas variopintas.
    La diferencia que guarda con el lema \ref{lema:A_5_uniformemente_denso_compactos} 
    es que si lo que estamos aproximando es una función medible, no tendremos una cota global del error,
    es decir \textbf{podemos asegurar que en casi todos los puntos esté muy próximo a la función que queremos aproximar}, pero pudiera ser que al evaluar un punto concreto el error sea muy grande.
    }

    \marginpar{\maginLetterSize
         \iconoClave \textcolor{darkRed}{
            \textbf{Reflexión sobre \ref{teo:2_4_rrnn_densas_M}}}
    }
    \marginpar{\maginLetterSize
        Por lo que acabamos de ver no se \textit{aproxima igual de bien} las funciones continuas a las medibles. La cuestión es la siguiente ¿la función que nosotros queremos aproximar con una red neuronal es continua o solo medible?
        A nivel práctico es ante todo desconocida, luego debemos 
        suponer las hipótesis menos restrictivas, es decir, que sea solo medible.
    }
