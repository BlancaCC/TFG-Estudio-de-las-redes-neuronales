...
descripción del método,  la mayoría de los datos cumplirán la propiedad de ortogonalidad y por tanto una buena 
...
Notemos que este coste es bastante menor al de realizar \textit{backpropagation} y que además éste debería de realizarse repetidamente pare mejorar el error considerablemente,  mientras que el nuestro tan solo una vez. 


\subsubsection{Observaciones }

Observemos nuestro algoritmo, para un mismo conjunto de entrenamiento  es capaz de producir infinitas soluciones 
diferentes ya que 
...
posible ver que se puede generalizar para funciones de activación más 
generales como definidas en \ref{table:funciones-de-activation}. 

\begin{itemize}
    \item Por el teorema \ref{teo:eficacia-funciones-activation} también será valido para \textbf{funciones de activación que sean afines a una que satisface \refeq{eq:method_inicializar_M}}. El proceso constructivo consistiría en que la red aprenda con la función que cumple satisface \refeq{eq:method_inicializar_M} y los pesos obtenidos transformarlos de acorde a la misma técnica que se aplica en la demostración del teorema \ref{teo:eficacia-funciones-activation}. 
    ...
    \item Para funciones del tipo anterior, pero asintóticas a $a,b \in \R$, con alguno de los extremos $a,b$ distintos de $0$ y $1$, bastará con realizar una transformación $f$ cumpliendo que $f(a)= 0$ y $f(b)= 1$ y aplicar teorema \ref{teo:eficacia-funciones-activation}. 
\end{itemize}
