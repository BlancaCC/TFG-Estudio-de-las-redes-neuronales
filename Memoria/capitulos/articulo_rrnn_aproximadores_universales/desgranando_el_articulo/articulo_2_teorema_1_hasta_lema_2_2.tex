...
\marginpar{\textbf{Qué se está probando en el teorema \ref{teo:TeoremaConvergenciaRealEnCompactosDefinicionesEsenciales}}
Si $G$ es una función continua, podemos aproximar tanto como queramos cualquier función
continua con elementos de $\pmcg$ es decir versiones algo más generales de redes neuronales. 
}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \marginpar{\textbf{Clave de la demostración del teorema \ref{teo:TeoremaConvergenciaRealEnCompactosDefinicionesEsenciales}}
        Se comprueba que \textit{operar} (sumar, multiplicar) dos redes neuronales generalizadas da otra red neuronal generalizada y que además dadas entradas distintas podemos conseguir salidas distintas. 
        Gracias a esto, el teorema de Weierstrass \ref{ch:TeoremaStoneWeiertrass} da una manera constructiva de aproximar las funciones continuas.
        }
    ...
    % Consecuencias de aplicar el teorema de Stone-Weierstrass
    \marginpar{\textcolor{darkRed}{\textbf{Implicaciones de usar el teorema de Stone-Weierstrass para democratizar las redes neuronales}}
    Tengamos presentes que la demostración ofrecida del teorema de Stone-Weierstrass 
    es constructiva, es decir existe un algoritmo bien definido con el que llegar estas estructuras. Por lo que si en las siguientes demostración se siguen con pruebas constructivas (como es el caso) habríamos encontrado un método alternativo al de 
    gradiente descendente \ref{sec:gradiente-descendente} con el seleccionar las redes neuronales. ¿Podríamos obtener algún beneficio de ello?
    Se razonará al respecto en la hipótesis \ref{hypothesis:pesos-iniciales}
    }
\end{proof}
...
\reversemarginpar
\marginpar{\textcolor{blue}{\textbf{Nueva hipótesis de optimización}}
...
no podemos plantear si aumentar el espacio de búsqueda a partir de transformaciones no lineales es más conveniente que hacerlo mediante neuronas.
Abordaremos esta cuestión en \ref{hypothesis:activation-function}.
}
...
\marginpar{\raggedright
    \textcolor{dark_green}{\textbf{Idea intuitiva equivalencia de funciones:}}}
    \marginpar{
...
\marginpar{\raggedright
    \textcolor{dark_green}{\textbf{Idea intuitiva distancia probabilidad:}}}
\marginpar{
...
\begin{proof} Para cada $n\in \N$ llamamos $g_n = \min\{|f_n - f|, 1|\}$.
    ...
    para cada natural $n$ con $n\geq n_0.$ 
          ...
          \marginpar{\raggedright
          \textcolor{dark_green}{\textbf{Idea intuitiva lema \ref{lema:A_1_C_es_denso_en_M}:}}
          }
          \marginpar{
